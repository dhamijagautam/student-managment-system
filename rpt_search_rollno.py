...
class Ui_rep_find_rollno(object):
    ...
    def export(self):
        ...
        if rn == '' or not rn.isnumeric():
            self.lbl_msg.setText('Roll no. should be numeric')
        else:
            ...
            cur1 = con.cursor()
            sql = 'select stu_id,name,stream,roll_no,phone_no from students inner join streams using(stream_id) where roll_no = ' + str(
                rn)
            cur1.execute(sql)
    ...
    def search(self):
        ...
        if rn == '' or not rn.isnumeric():
            ...
            con = mysql.connector.connect(host='localhost', user='root', passwd='gautam123', database='sms')
            cur1 = con.cursor()
            sql = 'select stu_id,name,stream,roll_no,phone_no from students inner join streams using(stream_id) where roll_no = '+str(rn)
            cur1.execute(sql)
            tbl_data=cur1.fetchall()
            tr=0
    ...
    def retranslateUi(self, rep_find_rollno):
        ...
        rep_find_rollno.setWindowTitle(_translate("rep_find_rollno", "Find student by roll no"))
        self.label.setText(_translate("rep_find_rollno", "Find Student By Roll no."))
        self.label_2.setText(_translate("rep_find_rollno", "Roll no."))
